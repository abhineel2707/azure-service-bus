...
namespace TopicsAndSubscriptions.WireTap
{
    internal class WireTapConsole
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            await managementClient.CreateSubscriptionAsync(new SubscriptionDescription(OrdersTopicPath, subscriptionName)
            {
                AutoDeleteOnIdle = TimeSpan.FromMinutes(5)
            });

            var subscriptionClient = new SubscriptionClient(ServiceBusConnectionString, OrdersTopicPath, subscriptionName);
            ...
            subscriptionClient.RegisterMessageHandler(InspectMessageAsync, messageHandlerOptions);
        ...
        private static async Task InspectMessageAsync(Message message, CancellationToken cancellationToken)
        {
